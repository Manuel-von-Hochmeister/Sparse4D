...
def _fill_trainval_infos(
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        # filter out boxes not visible in CAM_FRONT
        fc_token = sample["data"]["CAM_FRONT"]
        _, fc_boxes, _ = nusc.get_sample_data(fc_token, box_vis_level=BoxVisibility.ALL)
        all_fc_tokens = [box.token for box in fc_boxes]

        filtered_boxes = []
        for box in boxes:
            if box.token in all_fc_tokens:
        ...
        if not test:
            annotations = [
                ...
                for token in sample["anns"] if token in all_fc_tokens
            ]
            ...
            velocity = np.array(
                [nusc.box_velocity(token)[:2] for token in sample["anns"] if token in all_fc_tokens]
            )
            valid_flag = np.array(
                [
